		...
		<table class='table table-striped table-bordered' style='width:1600px;max-width:none'>
			<thead>
				<tr>
					...
					<?php echo $this->Paginator->sort('Rma','TOrmaOcorrenciaRma.orma_codigo')?>
					</td>
					<td class="input-xsmall">
					<?php echo $this->Paginator->sort('SM','TViagViagem.viag_codigo_sm')?>
					</td>
					<td class="input-xsmall">
					<?php echo $this->Paginator->sort('Embarcador','Embarcador.pjur_razao_social')?>
					</td>
					<td class="input-xsmall"><?php echo $this->Paginator->sort('Transportador','Transportador.pjur_razao_social')?></td>
					...
					<td class="input-xsmall"><?php echo $this->Paginator->sort('Status Viagem','TOrmaOcorrenciaRma.orma_viag_status')?></td>
					<td class="input-xsmall"><?php echo $this->Paginator->sort('Prioridade','TTrmaTipoRma.trma_prioridade')?></td>
				</tr>
			</thead>
			<tbody>
				<?php foreach ($dados as $key => $value): ?>
					...
					<tr> 
						...
						<td class='input-medium ellipsis' title="<?= $value[0]['orma_viag_status'] ?>">				<?= (isset($status_viagem[$value[0]['orma_viag_status']]) ? $status_viagem[$value[0]['orma_viag_status']] : '') ?></td>
						<td><?= $value[0]['trma_prioridade'] ?></td>
					</tr>
				<?php endforeach ?>
			</tbody>
			<tfoot>
