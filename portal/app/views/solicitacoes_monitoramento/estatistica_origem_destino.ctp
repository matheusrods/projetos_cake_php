<div class='well'>
    <?php echo $this->BForm->create('RelatorioEstatisticoSm', array('autocomplete' => 'off', 'url' => array('controller' => 'solicitacoes_monitoramento', 'action' => 'estatistica_origem_destino'))) ?>
        <div class="row-fluid inline">
            ...
            <?php echo $this->BForm->input('tipo_estatistica', array('label' => array('class' => 'radio inline'), 'div' => false, 'legend' => false, 'class' => 'radio inline', 'type' => 'radio', 'options' => array(1 => 'Origem', 2 => 'Destino'))); ?>
        </div>
...
<table class='table table-striped tablesorter'>
    <thead>
        <th><?php echo $this->Html->link('Cidade', 'javascript:void(0)') ?></th>
        <th class='numeric'><?php echo $this->Html->link('Quantidade de SMs', 'javascript:void(0)') ?></th>
    </thead>
    <?php $totalRegistros = 0; ?>
        ...
        <?php foreach ($data as $dados): ?>
            <tr>
                ...
                {
                    if($tipo_empresa == 4){
                        $tipo_do_cliente = "Transportador";
                    }else{
                        $tipo_do_cliente = "Embarcador";
                    }
                }                    
...
<?php echo $this->Javascript->codeBlock("
    function consulta_geral_sm( cida_codigo ) {
        var form = document.createElement('form');
        var form_id = ('formresult' + Math.random()).replace('.','');
        var tipo_estatistica = '{$this->data['RelatorioEstatisticoSm']['tipo_estatistica']}';
        ...
        if( tipo_estatistica == 1 ) {
            field = document.createElement('input');
            field.setAttribute('name', 'data[RelatorioSmConsulta][cida_codigo_origem]');
            field.setAttribute('value', cida_codigo);
            field.setAttribute('type', 'hidden');       
            form.appendChild(field);
        } else {
            field = document.createElement('input');
            field.setAttribute('name', 'data[RelatorioSmConsulta][cida_codigo_destino]');
            field.setAttribute('value', cida_codigo);
            field.setAttribute('type', 'hidden');       
            form.appendChild(field);
        }        
