...
<?php endif; ?>
<div class='well'>
    <strong>Transportador: </strong><?php echo $cliente['Cliente']['razao_social'] ?>
	<?php if (isset($cliente_embarcador['ClientEmpresa']['Raz_Social'])): ?>
		<strong>Embarcador: </strong><?php echo $cliente_embarcador['ClientEmpresa']['Raz_Social'] ?>
	<?php else: ?>
	...
	<table class='table table-striped tablesorter'>
        ...
        <?php $totalRegistros = 0; foreach ($data as $dados): ?>
            <tr>
                ...
                <td class='numeric'>
                    <?php echo $this->Html->link($dados[0]['total'] , 'javascript:void(0)', array( 'onclick' => "estatistica_rma_por_embarcador_gerador('{$this->data['Recebsm']['data_inicial']}','{$this->data['Recebsm']['data_final']}','{$cliente['Cliente']['codigo']}', '{$this->data['Recebsm']['cliente_embarcador']}','{$this->data['Recebsm']['cliente_transportador']}','{$dados['MRmaOcorrencia']['codigo_gerador_ocorrencia']}')" )); ?>
                </td>
            </tr>
