<?php if(!empty($grupos_riscos)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($grupos_riscos as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['GrupoRisco']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['GrupoRisco']['descricao'] ?></td>
                <td class="input-large"><?php echo $dados['GrupoRiscoExterno']['codigo_externo'] ?></td>
                <td>    
                    <?php if($dados['GrupoRisco']['ativo']== 0): ?>
                        <span class="badge-empty badge badge-important" title="Desativado"></span>
                    <?php elseif($dados['GrupoRisco']['ativo']== 1): ?>
                        <span class="badge-empty badge badge-success" title="Ativo"></span>
                    <?php endif; ?>
                
                    <?php echo $this->Html->link('', array('action' => 'editar_externo', $codigo_cliente_filtro, $dados['GrupoRisco']['codigo'], $dados['GrupoRiscoExterno']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar Código Externo')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['GrupoRisco']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'grupos_riscos/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
