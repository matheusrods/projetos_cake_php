<?php if(!empty($atribuicoes_cargos)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($atribuicoes_cargos as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['AtribuicaoCargo']['codigo'] ?></td>
                <td><?php echo $dados['AtribuicaoCargo']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['AtribuicaoCargo']['codigo']}','{$dados['AtribuicaoCargo']['ativo']}')"));?>

                <?php if($dados['AtribuicaoCargo']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['AtribuicaoCargo']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['AtribuicaoCargo']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['AtribuicaoCargo']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'atribuicoes_cargos/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
