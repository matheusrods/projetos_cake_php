...
<?php if(!empty($risco_atributo_detalhe)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($risco_atributo_detalhe as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['RiscoAtributoDetalhe']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['RiscoAtributoDetalhe']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatusRiscoAtributoDetalhe('{$dados['RiscoAtributoDetalhe']['codigo']}','{$dados['RiscoAtributoDetalhe']['ativo']}')"));?>

                <?php if($dados['RiscoAtributoDetalhe']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['RiscoAtributoDetalhe']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['RiscoAtributoDetalhe']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatusRiscoAtributoDetalhe(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'riscos_atributos_detalhes/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
