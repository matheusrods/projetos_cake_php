<?php if(!empty($atribuicoes_exames)):?>
    <?php echo $paginator->options(array('update' => 'div.lista')); ?>
    <table class="table table-striped">
        <thead>
            <tr>
               <th class="input-medium">Código</th>
            <th class="input-xxlarge">Atribuição</th>
            <th class="input-xxlarge">Exame</th>
            <th class="acoes" style="width:75px">Ações</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($atribuicoes_exames as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['AtribuicaoExame']['codigo'] ?></td>
                <td><?php echo $dados['Atribuicao']['descricao'] ?></td>
                <td><?php echo $dados['Exame']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['AtribuicaoExame']['codigo']}','{$dados['AtribuicaoExame']['ativo']}')"));?>

                <?php if($dados['AtribuicaoExame']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['AtribuicaoExame']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['AtribuicaoExame']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['AtribuicaoExame']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'atribuicoes_exames/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
