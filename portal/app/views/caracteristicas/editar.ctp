			...
			<tr>
				<td>
					...
					<div class="sub hide">
						...
						<div class="margin-left-30 margin-top-20"><?php echo $questao['label'] ?></div>
						<?php foreach ($questao['Respostas'] as $key3 => $resposta) { ?>
						<div class="margin-left-60">
							<?php if(in_array($key3, $this->data['Caracteristica']['respostas'])) { ?>
							<script type="text/javascript">$(document).ready(function() { abre_aba($('#resposta'+'<?php echo $i?>')); });</script>
							<?php } ?>
							<?php echo $this->Form->input('Caracteristica.respostas.'.$key3, array('type' => 'checkbox', 'style' => 'margin-top: 5px', 'id' => 'resposta'.$i, 'value' => $key3, 'label' => $resposta)); ?>
						</div>
						<?php $i++; } ?>
						<?php } ?>
					</div>
...
<script type="text/javascript">
	...
	function abre_aba(este) {
		este.parents('td').find('.open').find('i').removeClass('icon-chevron-right').addClass('icon-chevron-down');
		este.parents('td').find('.open').parent().find('.sub').removeClass('hide');
	}
</script>