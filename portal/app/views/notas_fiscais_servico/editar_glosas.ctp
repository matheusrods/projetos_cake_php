<div class="modal-dialog modal-sm" style="position: static;">
	<div class="modal-content" id="modal_data">
		...
		<div class="modal-header"><h3>Editar Glosa</h3></div>
		...
		<div class="modal-body" style="max-height: 400px; font-size: 15px;">
			<div class="row-fluid inline">
				<?php echo $this->BForm->create('NotaFiscalServico',array('url' => array('controller' => 'notas_fiscais_servico', 'action' => 'modal_glosas'), 'enctype' => 'multipart/form-data')); ?>
					<?php echo $this->BForm->input('codigo_glosa', array('type' => 'hidden', 'value' => $codigo_glosa)) ?>
					...
					<?php echo $this->BForm->input('codigo_itens_pedidos_exames', array('list'=> 'exames','value' => isset($dados_glosa[$glosas['Glosas']['codigo_itens_pedidos_exames']]['descricao']) ? $dados_glosa[$glosas['Glosas']['codigo_itens_pedidos_exames']]['descricao'] : null, 'label' => 'Exame', 'class' => 'input-xlarge', 'empty' => 'Exames')); ?>
					<datalist id="exames">
			    	...
			    	<?= $this->BForm->input('valor', array('value' => $glosas['Glosas']['valor'], 'label' => 'Valor', 'class' => 'input-medium numeric moeda', 'maxlength' => 14)); ?>
			</div>
			<div class="row-fluid inline">
				<?php echo $this->BForm->input('data_glosa', array('value' => $glosas['Glosas']['data_glosa'], 'label' => 'Data da Glosa', 'type' => 'text', 'class' => 'datepickerjs date input-small form-control', 'multiple')); ?>
				<?php echo $this->BForm->input('data_vencimento', array('value' => $glosas['Glosas']['data_vencimento'], 'label' => 'Data de vencimento', 'type' => 'text', 'class' => 'datepickerjs date input-small form-control', 'multiple')); ?>
				<?php echo $this->BForm->input('data_pagamento', array('value' => $glosas['Glosas']['data_pagamento'], 'label' => 'Data de Pagamento', 'type' => 'text', 'class' => 'datepickerjs date input-small form-control', 'multiple')); ?>
				<?php echo $this->BForm->input('codigo_status_glosa', array('value' => $glosas['Glosas']['codigo_status_glosa'], 'label' => 'Status', 'class' => 'input-medium', 'default' => '','empty' => 'Status','placeholder' => 'Exames', 'options' => $glosas_status)); ?>
			</div>
			<div class="row-fluid inline">
				<?php echo $this->BForm->input('motivo_glosa', array('value' => $glosas['Glosas']['codigo_tipo_glosa'], 'label' => 'Motivo', 'class' => 'input-xlarge', 'empty' => 'Motivo','options' => $tipos_glosas)); ?>
			</div>
		...
		<div class="modal-footer">
	    	<div class="right">
	        	<a id="InserirDadosGlosa" onclick="save_edicao_glosas()" href="javascript:void(0);" class="btn btn-primary">SALVAR</a>
...
$(document).ready(function() {
	...
	function save_edicao_glosas() {
		...
		if (retorno == true){
			...
			$.ajax({
				url: baseUrl + 'notas_fiscais_servico/save_edicao_glosas',
				type: 'POST',
				dataType: 'json',
				data: {
			...
			.done(function(data) {			
				if(data.return == 0) {
					swal({
						type: 'warning',
						title: 'Atenção',
						text: 'Glosa não foi atualizada!',
					});
				} else { 	
					swal({
						type: 'success',
						title: 'Sucesso',
						text: 'Glosa editada com sucesso.'
					});
					glosas();
					location.reload();
				}
