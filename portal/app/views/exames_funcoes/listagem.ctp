<?php if(!empty($exames_funcoes)):?>
    ...
    <table class="table table-striped">
        <thead>
            ...
            <th class="input-xxlarge">Função</th>
            <th class="acoes" style="width:75px">Ações</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($exames_funcoes as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['ExameFuncao']['codigo'] ?></td>
                <td><?php echo $dados['Exame']['descricao'] ?></td>
                <td><?php echo $dados['Funcao']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['ExameFuncao']['codigo']}','{$dados['ExameFuncao']['ativo']}')"));?>

                <?php if($dados['ExameFuncao']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['ExameFuncao']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['ExameFuncao']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['ExameFuncao']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
     ...
     function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'exames_funcoes/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
