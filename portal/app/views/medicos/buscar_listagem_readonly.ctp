...
<table class="table table-striped">
	...
	<tbody>
		<?php foreach ($medicos as $key => $dados): ?>
			<tr class="resultado-tr" codigo="<?php echo $dados['Medico']['codigo'] ?>" crm="<?php echo $dados['Medico']['numero_conselho']; ?>" uf="<?php echo $dados['Medico']['conselho_uf']; ?>" nome="<?php echo $dados['Medico']['nome']; ?>" cpf="<?php echo $dados['Medico']['cpf']; ?>">
				...
				<td><?php echo empty($dados['Medico']['cpf']) ? " " : AppModel::formataCpf($dados['Medico']['cpf']) ?></td>
			</tr>
		<?php endforeach ?>	 
	</tbody>
	<tfoot>
	...
	<?php echo $this->Javascript->codeBlock("jQuery(document).ready(function() {
		...
		$('tr.resultado-tr').click(function() {
			...
			if(double){
				...
				var input_cpf_display = $('#{$input_cpf_display}');
				input_cpf_display.val(cpf).change().blur();
