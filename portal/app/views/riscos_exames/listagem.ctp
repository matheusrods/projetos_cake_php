<?php if(!empty($riscos_exames)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($riscos_exames as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['RiscoExame']['codigo'] ?></td>
                <td><?php echo $dados['Risco']['nome_agente'] ?></td>
                <td><?php echo $dados['Exame']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['RiscoExame']['codigo']}','{$dados['RiscoExame']['ativo']}')"));?>

                <?php if($dados['RiscoExame']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['RiscoExame']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['RiscoExame']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['RiscoExame']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'riscos_exames/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
