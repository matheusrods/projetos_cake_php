<?php if(!empty($exames)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($exames as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['Exame']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['Exame']['descricao'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['Servico']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['Exame']['codigo']}','{$dados['Exame']['ativo']}')"));?>

                <?php if($dados['Exame']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['Exame']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['Exame']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['Exame']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'exames/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
    ...
    bloquearDiv(div);
    div.load(baseUrl + 'exames/listagem/' + Math.random());
}
");
?>
