        	...
        	<?php foreach ($dados_exames as $key => $exame) : ?>
                ...
                <tr>
                    ...
                    <td class="input-xlarge" style="background: <?php echo $color ?>;">
                    	...
                    	<?php else : ?>
                    		<?php echo $this->BForm->input('codigo_tipos_exames_pedidos', array('label' => false, 'class' => 'form-control uf input-large', 'style' => 'text-transform: uppercase;', 'default' => (isset($exame['tipo']) && $exame['tipo'] ? $exame['tipo'] : '') , 'options' => $lista_tipos_exames_outro, 'onchange' => 'atualiza_tipo(this, '. $key .', '.$codigo_funcionario_cliente.', '.$codigo_cliente.');')); ?>
                    	<?php endif; ?>
                    </td>
                    <td class="input-medium" style="background: <?php echo $color ?>; text-align: center;">                        
                        <?php if(!isset($exame['tipo'])) : ?>
                    	   <a href="javascript:void(0);" onclick="removeExameGrupo(<?php echo $key; ?>, <?php echo $codigo_cliente; ?>, <?php echo $codigo_funcionario_cliente; ?>, <?php echo $codigo_grupo_economico; ?>, this); " class="icon-trash"></a>
                        <?php endif; ?>
                    </td>
            	</tr>
            <?php endforeach ?>
