...
if ($this->Session->read('Message.flash.params.type') == MSGT_SUCCESS):
        echo $this->Javascript->codeBlock("close_dialog('{$this->Buonny->flash()}');atualizaListaClientesProdutosFinanceiro('{$codigo_cliente}');");        
    exit;
endif;
?>
...
<?php
    echo $bajax->form('ClienteProduto', array('url' => array('action' => 'atualizar_status_financeiro', $this->data['ClienteProduto']['codigo'], $codigo_cliente)));
?>
