...
<?php if(!empty($dados)): ?>
    ...
    <table class='table table-striped tablesorter'>
        ...
        ?>
            <tr>
                <td class="input-mini"><?php echo $this->Html->link($item['Tranpcc']['ccusto'] , 'javascript:void(0)', array( 'onclick' => "listar_titulos_pagos_por_centro_custo('Tranpag','{$item['Tranpcc']['ccusto']}','{$item['CentroCusto']['descricao']}','{$this->data['Tranpag']['data_inicial']}','{$this->data['Tranpag']['data_final']}','{$this->data['Tranpag']['grupo_empresa']}','{$this->data['Tranpag']['empresa']}')" )) ?></td>
