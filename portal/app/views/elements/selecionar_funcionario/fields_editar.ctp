						...
						<div class="inputs-config span<?php echo $subquestao['span'] ?> hides <?php  echo $hide ?>" style="margin-left: 0; margin-right: 1%">
							<div class="checkbox-canvas if-booleano">
								...
								switch ($subquestao['tipo']) {
									...
									// cria o input
									echo $this->BForm->input('FichaClinicaResposta.'.$subquestao['codigo'].'_resposta', array('type' => 'radio', 'options' => (array)json_decode($subquestao['conteudo']), 'data-open' => $openMenu, 'hiddenField' => false, 'legend' => false, 'required' => $required, 'suboption' => 1, 'data-option' => $subquestao['opcao_exibe_label']));
									...
									if(!empty($subquestao['parentesco_ativo'])) {
										...
										echo $this->BForm->input('FichaClinicaResposta.campo_livre.'.$subquestao['codigo'], array('type' => 'text', 'style' => 'width: 95%', 'label' => false, 'placeholder' => $subquestao['campo_livre_label'], 'style' => 'margin-bottom: 0; margin-left:10px; margin-top: -6px;', 'div' => 'js-label '.((!empty($subquestao['opcao_exibe_label']) &&  empty($this->data['FichaClinicaResposta']['campo_livre'][$subquestao['codigo']]) )? 'hide' : '')));
									}
									break;
						...
						<div class="js-encapsulado">
							...
							<div class="inputs-config span12 hide" style="margin-left: 0; margin-right: 1%; display: block;">
								<div class="checkbox-canvas">
									<div class="row-fluid">
										<div class="span12">
											<?php echo $this->BForm->input('FichaClinicaResposta.cid10.'.$questao['codigo'].'.'.$key4.'.doenca', array('value' => $value['doenca'], 'class' => 'js-cid-10', 'label' => 'CID10', 'style' => 'width: 90%; margin-bottom: 0; margin-top: -6px', 'div' => 'control-group input text width-full padding-left-10', 'required' => $required, 'after' => '<span style="margin-top: -7px" class="btn btn-default '.(($key4 == $i)? 'js-add-cid' : 'js-remove-cid').' pointer pull-right" data-toggle="tooltip" title="'.(($key4 == $i)? 'Adicionar doença' : 'Remover doença').'"><i class="'.(($key4 == $i)? 'icon-plus' : 'icon-minus').'" ></i></span>')); ?>
										</div>	
									</div>
									<div class="row-fluid pull-left padding-left-10 margin-top-5">
										<div class="span4">
											<?php echo $this->BForm->input('FichaClinicaResposta.cid10.'.$questao['codigo'].'.'.$key4.'.farmaco', array('value' => $value['farmaco'], 'class' => 'js-farmaco', 'label' => false, 'placeholder' => 'Fármaco', 'div' => array('style' => 'width: 95%'))) ?>
										</div>
										<div class="span4">
											<?php echo $this->BForm->input('FichaClinicaResposta.cid10.'.$questao['codigo'].'.'.$key4.'.posologia', array('value' => $value['posologia'], 'label' => false, 'class' => 'js-posologia', 'placeholder' => 'Posologia', 'div' => array('style' => 'width: 95%'))) ?>
										</div>
										<div class="span4">
											<?php echo $this->BForm->input('FichaClinicaResposta.cid10.'.$questao['codigo'].'.'.$key4.'.dose_diaria', array('value' => $value['dose_diaria'], 'label' => false, 'class' => 'js-dose_diaria', 'placeholder' => 'Dose diária', 'div' => array('style' => 'width: 95%'))) ?>
										</div>
									</div>
								</div>
							</div>
					...
					if($(this).attr('data-open')) {
						...
						if(!isNaN(parseInt(this.value)) &&  $(this).attr('suboption') != 1) {	
							if(parseInt(this.value) > 0) {
								$(this).parents('.subgroup-question').find('.inputs-config.hides').show();
							} else {
								$(this).parents('.subgroup-question').find('.inputs-config.hides').hide();
							}
						}
						if($(this).attr('data-option')) {
							if($(this).attr('data-option') == this.value) {
