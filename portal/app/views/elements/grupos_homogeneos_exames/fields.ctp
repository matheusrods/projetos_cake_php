...
<div class='actionbar-right'>
	...
	<table class="table table-striped" id="setor_cargo">
        ...
        <tbody>
        	<?php if(isset($edit_mode) && $edit_mode == 1):?>
	        	<?php if(isset($this->data['GrupoHomogeneoExameDetalhe']) && !empty($this->data['GrupoHomogeneoExameDetalhe'])):?>
		        	<?php foreach ($this->data['GrupoHomogeneoExameDetalhe'] as $key => $dados): ?>
						<tr id="linha">
							<td class="input-xxlarge">
								<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.'.$key.'.codigo', array('class' => 'grupo_homogeneo_campo', 'type' => 'hidden')); ?>
								<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.'.$key.'.codigo_setor', array('class' => 'grupo_homogeneo_campo', 'type' => 'hidden')); ?>
								<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.'.$key.'.codigo_cargo', array('class' => 'grupo_homogeneo_campo', 'type' => 'hidden')); ?>
 ...
 <div class='form-actions'>
	 ...
	 <?= $html->link('Voltar', array('controller' => 'grupos_homogeneos_exames', 'action' => 'index', $codigo_cliente, $referencia), array('class' => 'btn')); ?>
</div>
...
<div style="display:none;">
	<table id="modelo_campos">
		<tr id="linha">
			<td class="input-xxlarge">
				<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.x.codigo', array('class' => 'grupo_homogeneo_campo', 'type' => 'hidden')); ?>

				<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.x.codigo_setor', array('options' => $setor, 'label' => false, 'class' => 'input-xlarge grupo_homogeneo_campo', 'empty' => 'Selecione')); ?>
			</td>
			<td class="input-xxlarge">
				<?php echo $this->BForm->input('GrupoHomogeneoExameDetalhe.x.codigo_cargo', array('options' => $cargo, 'label' => false, 'class' => 'input-xlarge grupo_homogeneo_campo', 'empty' => 'Selecione')); ?>
			</td>
			<td class="acoes"><?php echo $this->Html->link('', 'javascript:void(0)', array('class' => 'icon-trash', 'title' => 'Excluir', 'onclick' => 'excluirItem(this)')) ?></td>
		</tr>
	</table>
