<div class='well'>
	...
	<div id="sub_fitro" class="row-fluid inline">
		<?php echo $bajax->form('Atestado', array('autocomplete' => 'off', 'url' => array('controller' => 'filtros', 'action' => 'filtrar', 'model' => 'Atestado', 'element_name' => 'quantitativo_por_cid'), 'divupdate' => '.form-procurar')) ?>
...
<?php echo $this->Javascript->codeBlock('
    jQuery(document).ready(function(){
		...
		$("#ClienteImplantacaoCodigoUnidade").change(function() {
			$.get( baseUrl + "filtros/limpar/model:Atestado/element_name:quantitativo_por_cid/" + Math.random(), atualizaLista($(this).val()) );
		});
		
		if($("#ClienteImplantacaoCodigoUnidade").val() != ""){
			atualizaLista( $("#ClienteImplantacaoCodigoUnidade").val() );
		}
		...
		bloquearDiv(div);
		$("#sub_fitro").load(baseUrl + "quantitativo_por_cid/sub_filtro_cliente_funcionario/" + $("#codigo_grupo_economico").val() + "/" + $("#ClienteImplantacaoCodigoUnidade").val()  + "/" + Math.random());
		div.load(baseUrl + "quantitativo_por_cid/listagem/" + $("#codigo_grupo_economico").val() + "/" + Math.random());
    });
		
	function atualizaLista(codigo_unidade) {
		$.ajax({
	        type: "POST",
	        url: "/portal/quantitativo_por_cid/retorna_codigo_grupo_economico",
	        dataType: "json",
	        ...
	        success: function(json) {
				if(json.codigo_grupo_economico) {
					$("#sub_fitro").load(baseUrl + "quantitativo_por_cid/sub_filtro_cliente_funcionario/" + json.codigo_grupo_economico + "/" + codigo_unidade + "/" + Math.random());
