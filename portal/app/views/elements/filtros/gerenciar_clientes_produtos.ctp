<div class="well">
  <?php echo $bajax->form('Cliente', array('autocomplete' => 'off', 'url' => array('controller' => 'filtros', 'action' => 'filtrar', 'model' => 'Cliente', 'element_name' => 'gerenciar_clientes_produtos'), 'divupdate' => '.form-procurar')) ?>
...
<?php echo $this->Javascript->codeBlock('
    function carregar_configuracao_mopp(codigo_cliente) {
       var html = "/portal/clientes_produtos/configuracoes_mopp/"+codigo_cliente+"/"+Math.random();
       open_dialog(html,"Configurações MOPP",900);
    }

    jQuery(document).ready(function(){
        
        setup_datepicker();
        atualizaListaClientes("gerenciar_clientes_produtos");
        jQuery("#limpar-filtro").click(function(){
            bloquearDiv(jQuery(".form-procurar"));
            jQuery(".form-procurar").load(baseUrl + "/filtros/limpar/model:Cliente/element_name:gerenciar_clientes_produtos/" + Math.random())
        });
