<div class='well'>
	...
	<div id="sub_fitro" class="row-fluid inline">
		<?php echo $bajax->form('ClienteFuncionario', array('autocomplete' => 'off', 'url' => array('controller' => 'filtros', 'action' => 'filtrar', 'model' => 'ClienteFuncionario', 'element_name' => 'atestados_funcionarios_cliente'), 'divupdate' => '.form-procurar')) ?>
			...
			<?php echo $this->BForm->input('ativo', array('value' => (isset($ativo) ? $ativo : '1'), 'class' => 'input-small', 'label' => 'Status', 'options' => $lista_status, 'id' => 'ativo','style' => 'width:100px', 'empty' => 'Ativos')); ?>
...
<?php echo $this->Javascript->codeBlock('
    jQuery(document).ready(function(){
		...
		if(!$("#codigo_grupo_economico").val()) {
	        ...
	        bloquearDiv(div);
			$("#sub_fitro").load(baseUrl + "atestados/sub_filtro_cliente_funcionario/" + $("#codigo_grupo_economico").val() + "/" + $("#ClienteImplantacaoCodigoUnidade").val()  + "/" + Math.random());
		}
		
		$("#ClienteImplantacaoCodigoUnidade").change(function() {
			$.get( baseUrl + "filtros/limpar/model:ClienteFuncionario/element_name:atestados_funcionarios_cliente/" + Math.random(), atualizaLista($(this).val()) );
		});
	...
	function atualizaLista(codigo_unidade) {
		$.ajax({
	        type: "POST",
	        url: "/portal/atestados/retorna_codigo_grupo_economico",
	        dataType: "json",
	        ...
	        success: function(json) {
				if(json.codigo_grupo_economico) {
					$("#sub_fitro").load(baseUrl + "atestados/sub_filtro_cliente_funcionario/" + json.codigo_grupo_economico + "/" + codigo_unidade + "/" + Math.random());
