<div class='well'>
    ...
    <div id='filtros'>
        <?php echo $this->Bajax->form('TEeveEstatisticaEvento', array('autocomplete' => 'off', 'url' => array('controller' => 'filtros', 'action' => 'filtrar', 'model' => 'TEeveEstatisticaEvento', 'element_name' => 'estatistica_eventos'), 'divupdate' => '.form-procurar')) ?>
        <div class="row-fluid inline">
        <?= $this->element('/estatistica_eventos/filtros') ?>
        </div>
        <span class="label label-info">Agrupamento:</span>
        <div id='agrupamento'>
            <?php echo $this->BForm->input('agrupamento', array('type' => 'radio', 'options' => $agrupamentos, 'default' => 1, 'legend' => false, 'label' => array('class' => 'radio inline input-small'))) ?>   
        </div>    
        <div class="row-fluid inline">
...
<?php echo $this->Javascript->codeBlock('
    jQuery(document).ready(function(){
        setup_datepicker();
        '.(isset($filtrado) && ($filtrado) ? 'var div = jQuery("div.lista");bloquearDiv(div);div.load(baseUrl + "estatistica_eventos/listagem/" + Math.random());':'').'

        jQuery("#limpar-filtro").click(function(){
            bloquearDiv(jQuery(".form-procurar"));
            jQuery(".form-procurar").load(baseUrl + "/filtros/limpar/model:TEeveEstatisticaEvento/element_name:estatistica_eventos/" + Math.random())
            jQuery(".lista").empty();
