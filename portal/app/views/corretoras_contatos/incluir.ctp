<?php
    if($session->read('Message.flash.params.type') == MSGT_SUCCESS){
        ...
        echo $javascript->codeBlock("carrega_contatos_cliente('{$this->passedArgs[0]}', 'div#contatos-corretora');close_dialog();");
        exit;
    }
?>


<?php echo $this->Bajax->form('CorretoraContato',array('url' => array('controller'=>'corretoras_contatos','action' => 'incluir', $this->passedArgs[0]))) ?>
 

<div class="row-fluid inline">
    <?php echo $this->BForm->hidden('codigo_corretora',array('value'=>$this->data['CorretoraContato']['codigo_corretora'])) ?>
...
<?php //echo $this->Buonny->link_js('corretoras'); ?>
<?php echo $javascript->codeBlock(
    "jQuery(document).ready(function(){
        tipoRetorno = jQuery('#CorretoraContatoCodigoTipoRetorno');
        tipoRetorno.change(
            function(){
                var descricao = jQuery('input#CorretoraContatoDescricao');
