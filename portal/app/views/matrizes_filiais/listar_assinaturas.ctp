<?php if (count($dados)): ?>
    <div style="margin-bottom:20px;">
        <h4>Liberação: Matriz / Filial</h4>
    </div>
    <table class='table cliente-produto'>
    	<thead>
    		<th class='input-medium'>Matriz</th>
            <th class='input-medium'>Filial</th>
    	...
    	<tbody>
    		<?php foreach($dados as $assinatura): ?>
                ...
                ?>
    			<tr>
                    <td><?php echo $assinatura['MatrizFilial']['codigo_cliente_matriz'].' - '.
                $assinatura[0]['nome_matriz'] ?></td>
                    <td><?php echo $assinatura['MatrizFilial']['codigo_cliente_filial'].' - '.
                $assinatura[0]['nome_filial'] ?></td>
                    <td><?php echo $assinatura['Produto']['descricao'] ?></td>
                    <td><?php echo $assinatura['MatrizProdutoPagador']['codigo_cliente_pagador'].' - '.
