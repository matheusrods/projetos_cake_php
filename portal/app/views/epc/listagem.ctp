<?php if(!empty($epc)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($epc as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['Epc']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['Epc']['nome'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatusEpc('{$dados['Epc']['codigo']}','{$dados['Epc']['ativo']}')"));?>

                <?php if($dados['Epc']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['Epc']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['Epc']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatusEpc(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'epc/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
