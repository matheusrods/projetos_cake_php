...
<?php echo $this->BForm->create('TVeicVeiculo', array('type' => 'POST', 'url' => array('controller' => 'Veiculos','action' => 'alterar2',$this->data['Cliente']['codigo'],$this->data['TVeicVeiculo']['veic_placa'])));?>
...
<div class='row-fluid inline'>
	...
	<?php echo $this->BForm->input('TMvecModeloVeiculo.mvec_mvei_codigo', array('label' => 'Fabricante','class' => 'input-large fabricante', 'empty' => 'Selecione um Fabricante', 'options' => $fabricantes)) ?>
...
<div class='row-fluid inline'>
	...
	<?php echo $this->BForm->input('veic_status', array('class' => 'input-small', 'label' => 'Status', 'empty' => false, 'options' => $status)) ?>
	<?php if($cliente_transportador): ?>
		<?php echo $this->BForm->hidden('TVtraVeiculoTransportador.vtra_tran_pess_oras_codigo') ?>
		<?php echo $this->BForm->input('TVtraVeiculoTransportador.vtra_tip_cliente', array('label' => 'Tipo de Veículo do Cliente','class' => 'input-medium', 'type' => 'text', 'maxlength'=>10)) ?>
		<?php if ($bloqueia_edicao): ?>
			<?php echo $this->BForm->hidden('TVtraVeiculoTransportador.vtra_refe_codigo_origem'); ?>
			<?php echo $this->BForm->input('TVtraVeiculoTransportador.vtra_refe_codigo_origem_visual', array('class' => 'input-large', 'label' => 'Alvo Origem', 'readonly'=>true)); ?>
		<?php else: ?>
			<?php echo $this->Buonny->input_referencia($this, '#ClienteCodigo', 'TVtraVeiculoTransportador', 'vtra_refe_codigo_origem', FALSE, 'Alvo Origem', TRUE) ?>
		<?php endif; ?>
</div>
<div class='row-fluid inline'>
...
<div class='row-fluid inline'>
	<?php else: ?>
		...
		<?php echo $this->BForm->input('TVembVeiculoEmbarcador.vemb_tip_cliente', array('label' => 'Tipo de Veículo do Cliente','class' => 'input-medium', 'type' => 'text')) ?>
		<?php if ($bloqueia_edicao): ?>
			<?php echo $this->BForm->hidden('TVembVeiculoEmbarcador.vemb_refe_codigo_origem'); ?>
			<?php echo $this->BForm->input('TVembVeiculoEmbarcador.vemb_refe_codigo_origem_visual', array('class' => 'input-large', 'label' => 'Alvo Origem', 'readonly'=>true)); ?>
		<?php else: ?>
...
<div class='row-fluid inline'>	
	<?php echo $this->BForm->input('veic_adesivado',array('type' => 'checkbox', 'label' => 'Veículo Adesivado', 'div' => array('class' => 'control-group input checkbox checkbox-parent-with-label') ));?>	
</div>
<div class='row-fluid inline'>
<div id='caminhao' >
	...
	<div class='row-fluid inline'>
		...
		<?php echo $this->BForm->input('TVtecVersaoTecnologia.vtec_codigo', array('label' => 'Versão','class' => 'tecnologia','empty' => 'Versão da Tecnologia', 'options' => $versoes)) ?>
		<?php echo $this->BForm->input('TTermTerminal.term_numero_terminal', array('label' => 'Numero', 'class' => 'input-medium tecnologia','maxlength' => 15)) ?>
		<label>&nbsp;</label>
	...
	<div class='row-fluid inline'>
		<?php if($cliente_transportador): ?>
			<?php echo $this->BForm->input('TVtraVeiculoTransportador.vtra_consumo_medio', array('label' => 'Consumo Médio', 'empty' => 'Consumo Médio', 'class' => 'moeda_com_decimal numeric input-small')) ?>
			<?php echo $this->BForm->input('TVtraVeiculoTransportador.vtra_eixos', array('label' => 'Qtd. Eixos', 'empty' => 'Qtd. Eixos', 'class' => 'just-number numeric input-mini')) ?>
		<?php else: ?>
			<?php echo $this->BForm->input('TVembVeiculoEmbarcador.vemb_consumo_medio', array('label' => 'Consumo Médio', 'empty' => 'Consumo Médio', 'class' => 'moeda_com_decimal numeric input-small')) ?>
			<?php echo $this->BForm->input('TVembVeiculoEmbarcador.vemb_eixos', array('label' => 'Qtd. Eixos', 'empty' => 'Qtd. Eixos', 'class' => 'just-number numeric input-mini')) ?>
		<?php endif; ?>
