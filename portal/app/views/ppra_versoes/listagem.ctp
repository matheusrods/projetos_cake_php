		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<th style="width:15%">Profissional Responsável</th>
			...
			<tbody>
				<?php foreach ($listagem as $key => $linha): ?>
					<tr>
						<td class="input-mini"><?= $linha['PpraVersoes']['codigo']; ?></td>
						<td><?= $linha['GrupoEconomico']['descricao']; ?></td>
						<td><?= $linha['PpraVersoes']['versao']; ?></td>
						<td>
							<?= empty($linha['Medicos']['nome']) ? 'Não consta' : $linha['Medicos']['nome']; ?>
						</td>
						<td><?= $linha['PpraVersoes']['inicio_vigencia_ppra']; ?></td>
						<td><?= $linha['PpraVersoes']['periodo_vigencia_ppra']; ?></td>
						<td><?= date('d/m/Y', strtotime($linha[0]['final_vigencia_ppra'])); ?></td>
						<td>
							<?= $html->link('', array('controller' => 'ppra_versoes', 'action' => 'imprimir_relatorio', $linha['PpraVersoes']['codigo'], $linha['PpraVersoes']['codigo_cliente_alocacao']), array('class' => 'icon-print','data-toggle' => 'tooltip', 'title' => 'Imprimir relatório')); 
						?>
					</td>
				</tr>
			<?php endforeach; ?>        
