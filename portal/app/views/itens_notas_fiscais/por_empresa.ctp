<?php if (empty($empresas)): ?>
	<div class='well'>
		<?php echo $this->BForm->create('Notaite', array('autocomplete' => 'off', 'url' => array('controller' => 'itens_notas_fiscais', 'action' => 'por_empresa'))) ?>
	...
	<?php if (count($natec_do_ano) > 0): ?>
		<table class='table table-striped table-bordered'>
			...
			<?php foreach ($empresas as $codigo => $empresa): ?>
				<?php $total_mes += (isset($natec_do_mes[$codigo]) ? $natec_do_mes[$codigo]['0']['total'] : 0) ?>
				<?php $total_ano += (isset($natec_do_ano[$codigo]) ? $natec_do_ano[$codigo]['0']['total'] : 0) ?>
				<?php $valor_mes = (isset($natec_do_mes[$codigo]) ? $this->Buonny->moeda($natec_do_mes[$codigo]['0']['total']) : '') ?>
				<?php $valor_ano = (isset($natec_do_ano[$codigo]) ? $this->Buonny->moeda($natec_do_ano[$codigo]['0']['total']) : '') ?>
				<?php if (!empty($valor_ano)): ?>
					<tr>
						<td><?= $empresa ?></td>
						<td class='numeric'><?= $this->Html->link($valor_mes, 'javascript:void(0)', array('onclick' => "por_produto('".LojaNaveg::GRUPO_NATEC."', '{$codigo}', 'mes', {$this->data['Notaite']['mes']}, {$this->data['Notaite']['ano']})")) ?></td>
						<td class='numeric'><?= $this->Html->link($valor_ano, 'javascript:void(0)', array('onclick' => "por_produto('".LojaNaveg::GRUPO_NATEC."', '{$codigo}', 'ano', {$this->data['Notaite']['mes']}, {$this->data['Notaite']['ano']})")) ?></td>
						<td class='action-icon'><?= $this->Html->link('', 'javascript:void(0)', array('onclick' => "comparativo_anual('{$this->data['Notaite']['ano']}','".LojaNaveg::GRUPO_NATEC."', '{$codigo}')", 'class' => 'icon-list-alt', 'title' => 'Comparativo Anual')) ?></td>
					</tr>
				<?php endif ?>
			<?php endforeach ?>
			<tfoot>
				<td>Total</td>
				<td class='numeric'><?= $this->Html->link($this->Buonny->moeda($total_mes), 'javascript:void(0)', array('onclick' => "por_produto('".LojaNaveg::GRUPO_NATEC."', '', 'mes', {$this->data['Notaite']['mes']}, {$this->data['Notaite']['ano']})")) ?></td>
				<td class='numeric'><?= $this->Html->link($this->Buonny->moeda($total_ano), 'javascript:void(0)', array('onclick' => "por_produto('".LojaNaveg::GRUPO_NATEC."', '', 'ano', {$this->data['Notaite']['mes']}, {$this->data['Notaite']['ano']})")) ?></td>
				<td class='action-icon'><?= $this->Html->link('', 'javascript:void(0)', array('onclick' => "comparativo_anual('{$this->data['Notaite']['ano']}','".LojaNaveg::GRUPO_NATEC."', '')", 'class' => 'icon-list-alt', 'title' => 'Comparativo Anual')) ?></td>
			</tfoot>
		</table>
	<?php endif ?>
...
<?= $this->Javascript->codeBlock("
	...
	function por_produto(grupo_empresa, empresa, tipo, mes, ano){
        ...
        form.setAttribute(\"target\", \"formresult\");
		form.setAttribute(\"method\", \"post\");
        form.setAttribute(\"action\", \"/portal/itens_notas_fiscais/por_produto/1\");
        field = document.createElement(\"input\");
