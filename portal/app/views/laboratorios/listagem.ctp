<?php if(!empty($laboratorios)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($laboratorios as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['Laboratorio']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['Laboratorio']['descricao'] ?></td>
                <td>
                <?php echo $this->Html->link('', 'javascript:void(0)',array('class' => 'icon-random troca-status', 'escape' => false, 'title'=>'Troca Status','onclick' => "atualizaStatus('{$dados['Laboratorio']['codigo']}','{$dados['Laboratorio']['ativo']}')"));?>

                <?php if($dados['Laboratorio']['ativo']== 0): ?>
                    <span class="badge-empty badge badge-important" title="Desativado"></span>
                <?php elseif($dados['Laboratorio']['ativo']== 1): ?>
                    <span class="badge-empty badge badge-success" title="Ativo"></span>
                <?php endif; ?>
                
                <?php echo $this->Html->link('', array('action' => 'editar', $dados['Laboratorio']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
        <tfoot>
            <tr>
                <td colspan = "10"><strong>Total</strong> <?php echo $this->Paginator->params['paging']['Laboratorio']['count']; ?></td>
            </tr>
        </tfoot>    
    </table>
    <div class='row-fluid'>
...
echo $this->Javascript->codeBlock("
    ...
    function atualizaStatus(codigo, status){
        $.ajax({
            type: 'POST',
            url: baseUrl + 'laboratorios/atualiza_status/' + codigo + '/' + status + '/' + Math.random(),
            beforeSend: function(){
