<div class = 'form-procurar'>
	<?= $this->element('/filtros/viagens_faturamento_subtotal') ?>
</div>
<div class='lista'></div>

<?php echo $this->Javascript->codeBlock("
	function listar_sms(pagador,embarcador,transportador,tipo){
	    ...
	    jQuery(\"#postlink #ViagemFaturamentoEmbarcador\").val(embarcador);
	    jQuery(\"#postlink #ViagemFaturamentoTransportador\").val(transportador);
	...
	function placas(codigo_cliente,tipo){
	    ...
	    jQuery(\"#postlink #ViagemFaturamentoTotalMesFaturamento\").val(jQuery('#ViagemFaturamentoSubtotalMesFaturamento').val());
	    jQuery(\"#postlink #ViagemFaturamentoTotalAnoFaturamento\").val(jQuery('#ViagemFaturamentoSubtotalAnoFaturamento').val());
	    var janela = window_sizes();
		window.open('', form_id, 'scrollbars=yes,menubar=no,height='+(janela.height-200)+',width='+(janela.width-80)+',resizable=yes,toolbar=no,status=no');
	    jQuery(\"#postlink form\").submit();
	}
