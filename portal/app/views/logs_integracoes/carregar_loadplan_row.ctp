<?php if($mensagem): ?>
<div class='erro'>
	<?php echo $this->BForm->error_menssage($mensagem) ?>
...
<?php else: ?>
	...
	<?php if(isset($this->data['Recebsm'])): ?>
	...
	<?php 
		if($loadUtilisado){
			if(isset($this->data['Recebsm'])){
				echo $this->Javascript->codeBlock('
					if(!confirm("Loadplan já utilizado. Deseja continuar?")){
						$("div#loadplan").html("");
					}
				');
			} else {
				echo $this->Javascript->codeBlock('
					if(!confirm("Loadplan já utilizado. Deseja continuar?")){
						$("div#loadplan").find("table.destino:last ").remove();
					}
				');
			}
		}
