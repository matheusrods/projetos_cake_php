...
?>
<?php echo $this->BForm->create('ReguladorRegiao', array('url' => array('controller' => 'reguladores_regioes','action' => 'incluir', $this->passedArgs[0])));?>
  ...
  <div style="width:500px;float:left">
    <div class="row-fluid inline">
      ...
      <div class="row-fluid inline">
        ...
        <?php echo $this->BForm->input('raio', array('label' => 'Raio (m)','class' => 'input-small just-number numeric','value' => '500')) ?>
      </div>
    <div class="row-fluid inline">
      <?php echo $this->BForm->input('cidade', array('label' => 'Cidade', 'class' => 'text-medium')) ?>
      <?php echo $this->BForm->input('prioridade', array('value' => 1,'label' => 'Prioridade','class' => 'input-small numeric')) ?>
    </div>
  </div>
...
$(document).ready(function(){   
  ...
  $("#SinistroLatitude").blur(function(){
    renderiza_marcador_mapa($("#ReguladorRegiaoLatitude").val(),$("#ReguladorRegiaoLongitude").val());
  });
  ...
  function initMap(myLatLng,raio) {  
    ...
    document.getElementById("submit").addEventListener("click", function() {
      ...
      function atualiza_raio(latLng){        
        bounds = new google.maps.LatLngBounds(
          new google.maps.LatLng(latLng.lat()-raio, latLng.lng()-raio),
          new google.maps.LatLng(latLng.lat()+raio, latLng.lng()+raio)
        );
