<?php if(!empty($setores)):?>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($setores as $dados): ?>
            <tr>
                <td class="input-mini"><?php echo $dados['Setor']['codigo'] ?></td>
                <td class="input-xxlarge"><?php echo $dados['Setor']['descricao'] ?></td>
                <td class="input-large"><?php echo $dados['SetorExterno']['codigo_externo'] ?></td>
                <td>    
                    <?php if($dados['Setor']['ativo']== 0): ?>
                        <span class="badge-empty badge badge-important" title="Desativado"></span>
                    <?php elseif($dados['Setor']['ativo']== 1): ?>
                        <span class="badge-empty badge badge-success" title="Ativo"></span>
                    <?php endif; ?>
                
                    <?php echo $this->Html->link('', array('action' => 'editar_externo', $codigo_cliente, $dados['Setor']['codigo'], $dados['SetorExterno']['codigo']), array('class' => 'icon-edit ', 'title' => 'Editar Código Externo')); ?>
                </td>
            </tr>
        <?php endforeach ?>
    </tbody>
