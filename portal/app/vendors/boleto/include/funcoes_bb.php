...
$valor = str_pad(str_replace('.', '', $dadosboleto["valor_boleto"]),10,"0", STR_PAD_LEFT);
//agencia é sempre 4 digitos
$agencia = str_pad($dadosboleto["agencia"], 4, "0", STR_PAD_LEFT);
...
if ($dadosboleto["formatacao_convenio"] == "8") {
	$convenio = str_pad($dadosboleto["convenio"], 8, "0", STR_PAD_LEFT);
	// Nosso número de até 9 dígitos
	$nossonumero = str_pad($dadosboleto["nosso_numero"], 9, "0", STR_PAD_LEFT);
...
if ($dadosboleto["formatacao_convenio"] == "7") {
	$convenio = str_pad($dadosboleto["convenio"], 7, "0", STR_PAD_LEFT);
	// Nosso número de até 10 dígitos
	$nossonumero = str_pad($dadosboleto["nosso_numero"], 10, "0", STR_PAD_LEFT);
	$dv=modulo_11("$codigobanco$nummoeda$fator_vencimento$valor$livre_zeros$convenio$nossonumero$carteira");
	$linha="$codigobanco$nummoeda$dv$fator_vencimento$valor$livre_zeros$convenio$nossonumero$carteira";
	if ($carteira == '17'){
	    $nossonumero = $convenio . $nossonumero ."-". modulo_11($convenio.$nossonumero);
...
if ($dadosboleto["formatacao_convenio"] == "6") {
	$convenio = str_pad($dadosboleto["convenio"], 6, "0", STR_PAD_LEFT);
	
	if ($dadosboleto["formatacao_nosso_numero"] == "1") {
		
		// Nosso número de até 5 dígitos
		$nossonumero = str_pad($dadosboleto["nosso_numero"], 5, "0", STR_PAD_LEFT);
	...
	if ($dadosboleto["formatacao_nosso_numero"] == "2") {
		...
		$nservico = "21";
		$nossonumero = str_pad($dadosboleto["nosso_numero"], 17, "0", STR_PAD_LEFT);
...
//Guarda inicial
?><img src=/portal/img/boleto/p.png width=<?php echo $fino?> height=<?php echo $altura?> border=0><img 
src=/portal/img/boleto/b.png width=<?php echo $fino?> height=<?php echo $altura?> border=0><img 
src=/portal/img/boleto/p.png width=<?php echo $fino?> height=<?php echo $altura?> border=0><img 
src=/portal/img/boleto/b.png width=<?php echo $fino?> height=<?php echo $altura?> border=0><img 
<?php
$texto = $valor ;
if((strlen($texto) % 2) <> 0){
...
?>
    src=/portal/img/boleto/p.png width=<?php echo $f1?> height=<?php echo $altura?> border=0><img 
<?php
    if (substr($f,$i,1) == "0") {
      $f2 = $fino ;
    }else{
...
?>
    src=/portal/img/boleto/b.png width=<?php echo $f2?> height=<?php echo $altura?> border=0><img 
<?php
  }
}
...
// Draw guarda final
?>
src=/portal/img/boleto/p.png width=<?php echo $largo?> height=<?php echo $altura?> border=0><img 
src=/portal/img/boleto/b.png width=<?php echo $fino?> height=<?php echo $altura?> border=0><img 
src=/portal/img/boleto/p.png width=<?php echo 1?> height=<?php echo $altura?> border=0> 
  <?php
} //Fim da função
