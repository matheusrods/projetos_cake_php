<?php
class ImportSmMonitoraShell extends Shell {
	...
	function selectGrupo($grupo){
		...
		*/
		...
		$dia = str_pad((31-$grupo),2,'0',STR_PAD_LEFT);
		$grupos[$grupo] = array(
			"201309{$dia} 23:00:00",
			"201309{$dia} 00:00:00"
		);
	...
	function main() {
		...
		$GPA = "embarcador.codigo <> '{$this->SmGpa->cliente_monitora}'";
		if(isset($this->args[1]) && $this->args[1] == 'S')
			$GPA = "embarcador.codigo = '{$this->SmGpa->cliente_monitora}' AND recebsm.sistema_origem = 'INTEGRACAO FTP'";

		$this->erro_arq = "log_viag_recuperacao_{$this->args[0]}.log";
		...
		$query = "SELECT 
		  ...
		  --FuncInicio.NOME AS funcionario_inicio,
		  --FuncFim.NOME AS funcionario_fim,
		...
		FROM monitora..recebsm 
		    ...
		    --LEFT JOIN Monitora..Acomp_Viagem AS AcomopInicio ON AcomopInicio.SM = recebsm.SM AND AcomopInicio.Tipo_Parada = '01'
		    --LEFT JOIN Monitora..Funcionarios AS FuncInicio ON FuncInicio.CODIGO = AcomopInicio.OPERADOR
		    --LEFT JOIN Monitora..Acomp_Viagem AS AcomopFim ON AcomopFim.SM = recebsm.SM AND AcomopFim.Tipo_Parada = '14'
		    --LEFT JOIN Monitora..Funcionarios AS FuncFim ON FuncFim.CODIGO = AcomopFim.OPERADOR
	...
	function inserirDadosViagem($viagem){
			...
			if($viagem['cliente_tipo'] == $this->SmGpa->cliente_monitora){
				echo "... GPA ...\n";
				$xml = $this->carregarSmGpa($viagem);
				if($xml){
					$index = 0;
					//var_dump($viagem['RecebsmAlvoDestino']);die;
					$ultimo_alvo = array();
					foreach ($viagem['RecebsmAlvoDestino'] as $value) {
						if(isset($xml['RecebsmAlvoDestino'][$index])){
							$xml['RecebsmAlvoDestino'][$index++]['itinerarios'] = $value['itinerarios'];
							$ultimo_alvo = $value;
						}
					}

					foreach ($xml['RecebsmAlvoDestino'] as  $key => $value) {
						if(!isset($value['itinerarios'])){
							if($value['refe_codigo'] == $viagem['RecebsmAlvoOrigem'][0]['refe_codigo'])
								$xml['RecebsmAlvoDestino'][$key]['itinerarios'] =  $viagem['RecebsmAlvoOrigem'][0]['itinerarios'];
							else {
								$xml['RecebsmAlvoDestino'][$key]['itinerarios'] =  $ultimo_alvo['itinerarios'];
							}
						}
							
					}
					
					$xml['RecebsmAlvoOrigem'][0]['itinerarios'] = $viagem['RecebsmAlvoOrigem'][0]['itinerarios'];

					$viagem['RecebsmAlvoOrigem']	= $xml['RecebsmAlvoOrigem'];
					$viagem['RecebsmAlvoDestino'] 	= $xml['RecebsmAlvoDestino'];
				}
			}
			...
			try{
				...
				$this->TRefeReferencia->verificaAlvosSM($viagem['RecebsmAlvoOrigem']);
				
				// VERIFICA SE O ALVO DE DESTINO FOI LOCALIZADO E INSERE
				$this->TRefeReferencia->verificaAlvosSM($viagem['RecebsmAlvoDestino']);
		...
		$dados = array( 
			...
			'RecebsmAlvoOrigem' => array(
									 	array( 
									 		...
									 		'numero' => null,
									 		'cep' => null,
									 		'cd' => ($viagem[0]['alvo_conhecido'] == 1 ? $viagem[0]['refe_depara'] : substr(Comum::trata_nome($viagem[0]['empresa']),0,20)), // depara
...
/*
SELECT 
    MIN(CAST(recebsm.sm AS INT)) AS DE,
	MAX(CAST(recebsm.sm AS INT)) AS ATE	  
FROM monitora..recebsm 
    INNER JOIN monitora..SmIntinerario ON SmIntinerario.SM = recebsm.sm 
    INNER JOIN Monitora..Client_Empresas AS transportador ON transportador.codigo = Recebsm.cliente
    LEFT JOIN Monitora..Client_Empresas AS embarcador ON embarcador.codigo = Recebsm.emprelacionada
    INNER JOIN Monitora..Caminhao ON Caminhao.Placa_Cam = RECEBSM.Placa
    INNER JOIN Monitora..Carreta ON Carreta.Placa_Carreta = RECEBSM.Placa_Carreta
    INNER JOIN Monitora..Motorista ON Motorista.Codigo = Recebsm.MotResp
    LEFT JOIN dbCorreios..gris_gerenciadora_risco ON gris_gerenciadora_risco.pjur_razao_social = recebsm.nome_gerenciadora
    LEFT JOIN dbCorreios..refe_referencia ON refe_referencia.refe_codigo = codigo_trafegus_refe_referencia 
    LEFT JOIN dbCorreios..ploc_pessoa_local ON ploc_pessoa_local.refe_codigo = refe_referencia.refe_codigo
...
WHERE recebsm.dta_receb BETWEEN '20130915 00:00:00' AND '20131003 23:59:59'
'11282203'recebsm.dta_receb BETWEEN '20130801 00:00:00' AND '20131031 23:59:59'
*/
