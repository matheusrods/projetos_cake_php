<?php 
class ApiFontesGeradorasController extends AppController {
    ...
    public function beforeFilter() {
        ...
        $this->BAuth->allow('consulta_fonte_geradora', 'valida_autorizacao');
    ...
    public function consulta_fonte_geradora()  {
        ...
        $fields = array('FonteGeradora.codigo', 'FonteGeradoraExterno.codigo_externo', 'FonteGeradora.nome as descricao');

        $this->FonteGeradora->bindModel(
            array('hasOne' => array(
                'FonteGeradoraExterno' => array(
                    'foreignKey' => 'codigo_fontes_geradoras', 
                    'conditions' => array('FonteGeradoraExterno.codigo_cliente' => $codCliente['Cliente']['codigo'])
                )
            )
        ), false);

        $join = array(
          'table' => 'RHHealth.dbo.fontes_geradoras_externo',
          'alias' => 'fontes_geradoras_externos',
          'type' => 'LEFT',
          'conditions' => 'fontes_geradoras_externos.codigo_fontes_geradoras = FonteGeradora.codigo',
        );

        $fonteGeradora = $this->FonteGeradora->find('all', array(
            'join' => $join,
            'fields' => $fields,
            'order' => 'FonteGeradora.codigo'
        ));

        $this->dados['fonte_geradora']  = '';

        if (!empty($fonteGeradora)) {
            //Percorre o array
            foreach ($fonteGeradora as $chave => $valor) :
                $item = new stdClass();
                $item->codigo = $valor['FonteGeradora']['codigo'];
                $item->codigo_externo = $valor['FonteGeradoraExterno']['codigo_externo'];
                $item->descricao = $valor[0]['descricao'];
                $this->dados['fonte_geradora'][$chave] = $item;
            endforeach;
