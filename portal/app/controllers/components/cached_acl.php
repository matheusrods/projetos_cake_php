 ...
 * @param string $aro ARO
 * @param string $aco ACO
 * @param boolean $aroPath Boolean to return only the path to the ARO or the full path to the permission.
 * @access private
 */
    function __cachePath($aro, $aco, $action, $aroPath = false) {
        if ($action != "*") {
            $_aco = $aco . '/' . $action;
        } else {
	        $_aco = $aco;
		...
		if ($aroPath) {
			$path = Inflector::slug($_aro);
        } else {
            $path = Inflector::slug($_aro) . '.' . Inflector::slug($_aco);
		}
 ...
 */    
    function __deleteCache($aro, $aco, $action) {
		//Limpa todo o cache, pois invertemos a ordem dos parametros no __cachePath.
        //Cache::delete('/', 'acl');
        $path = $this->__cachePath($aro, '*', null, true);
        Cache::delete($path, 'acl');
    }

	//exemplo de uso: $this->CachedAcl->clearCache(array('WgiContaUsuario'=>array('id'=>20)));
	function clearCache($aro) {
		$path = $this->__cachePath($aro, '*', null, true);
		Cache::delete($path, 'acl');
	}
}
