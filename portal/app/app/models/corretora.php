...
class Corretora extends AppModel {
    ...
    public function exportarCorretor(   $nome_empresa,
        ...
        $insertEmpresa = "  INSERT INTO RHHealth_vendas.dbo.empresa ( nome_empresa, cnpj, email ) 
                            VALUES ( '". $nome_empresa ."', '". $cnpj ."', '". $email ."' ); SELECT @@IDENTITY ";
        if( $success = $this->query( $insertEmpresa ) ){
            ...
            // Criar um usuario
            $dados['CorretorUsuarioVendas'] = array(    "codigo_empresa"    => $last_id,
                                                        "nome"              => $nome_usuario,
                                                        "email"             => $email,
                                                        "codigo_validacao"  => $codigo_validacao,
                                                        "senha"             => NULL,
                                                        "ativo"             => 0  ) ;            
            $success = parent::incluir( $dados );
        }
